...
const selection=JSON.parse(fs.readFileSync(path.join(root,'data','generated','global-runtime-task-selection.json'),'utf8'));
const unitTests=fs.readdirSync(path.join(root,'tests')).filter((name)=>name.endsWith('.test.js')).sort().map((name)=>`tests/${name}`);

function main(){
  return packageStage({
    stageId:'global-runtime-stage',
    artifactPrefix:'artifacts/global-runtime-stage/',
    generationHead:selection.generated_from_head,
    coldCommands:[
      {label:'node scripts/import-content.js',args:['scripts/import-content.js']},
      {label:'node scripts/audit-global-task-exceptions.js',args:['scripts/audit-global-task-exceptions.js']},

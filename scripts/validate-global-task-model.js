...
function main(){
  ...
  check('remaining_fit_task_count',fitting.remaining_task_count,508);
  check('independent_direct_fit_count',evaluation.tasks.filter((entry)=>entry.direct_fit).length,648);
  check('independent_exception_count',evaluation.tasks.filter((entry)=>!entry.direct_fit).length,3);
  check('remaining_direct_fit_count',fitting.class_summary.filter((entry)=>entry.class_id<=4).reduce((sum,entry)=>sum+entry.count,0),505);
  check('legacy_audit_selector_count',selection.selected_task_count,143);
  check('validated_baseline_count',directory.status_counts.validated,648);
  check('directory_status_counts',directory.status_counts,{runnable_pending_validation:3,validated:648});
  check('exception_cluster_task_count',exceptions.exception_task_count,3);
  check('browser_regression_task_count',content.entity_counts.tasks,651);

...
const coldCommands=[
  ...
  {label:'node --test tests/progression-source-golden.test.js',args:['--test','tests/progression-source-golden.test.js']},
  ...
  {label:'node --test tests/equipment-acquisition.test.js',args:['--test','tests/equipment-acquisition.test.js']},
...
function main(){const outputIndex=process.argv.indexOf('--output-root');return packageStage({stageId:'equipment-combat-stage',
  artifactPrefix:'artifacts/equipment-combat-stage/',coldCommands,outputRoot:outputIndex>=0?path.resolve(root,process.argv[outputIndex+1]):undefined});}

...
function main(){
  ...
  const temporaryRoot=fs.mkdtempSync(path.join(os.tmpdir(),'zhsh-two-package-runs-'));
  try{
    ...
    const finalRoot=path.join(root,'out','progression-stage');fs.mkdirSync(finalRoot,{recursive:true});
    ...
    const evidence={schema_version:1,result:'passed',independent_full_runs:2,same_clean_commit:true,zip_sha256:results[0].sha256,
    ...
    process.stdout.write(`${JSON.stringify({...evidence,final_zip:absolute(finalZip)},null,2)}\n`);return evidence;
  }finally{safeRemove(temporaryRoot);}
}

function packageOnce(outputRoot){const result=spawnSync(process.execPath,['scripts/package-global-recovery-stage.js','--output-root',outputRoot],
  {cwd:root,encoding:'utf8',windowsHide:true,maxBuffer:64*1024*1024});if(result.status!==0)throw new Error(`Independent package run failed:\n${result.stdout??''}${result.stderr??''}`);
  const payload=JSON.parse(result.stdout);const sha256=hash(fs.readFileSync(payload.zip_path));if(sha256!==payload.zip_sha256)throw new Error('Packager-reported ZIP hash mismatch');

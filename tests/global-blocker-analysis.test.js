...
test('global blocker analysis reproduces the accepted baseline and current 651-task partition',()=>{
  assert.equal(analysis.accepted_baseline.selected_task_count,82);
...
test('isolated module deltas and combination synergy are explicit rather than inferred',()=>{
  ...
  assert.equal(modules.get('training_session_continuation').actual_simulated_unlock_delta,66);
  assert.equal(modules.get('task_described_item_sources').actual_simulated_unlock_delta,0);
  assert.equal(modules.get('projected_task_entry_combat_state').actual_simulated_unlock_delta,0);
  assert.equal(analysis.current_result.combined_simulated_unlock_delta,61);
});

test('module deltas come from five persisted selector simulations',()=>{
  assert.equal(simulations.cases.baseline_all_modules_disabled.selected_task_count,82);
  assert.equal(simulations.cases.training_session_continuation.selected_task_count,148);
  assert.equal(simulations.cases.task_described_item_sources.selected_task_count,82);
  assert.equal(simulations.cases.projected_task_entry_combat_state.selected_task_count,82);

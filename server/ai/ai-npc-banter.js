 ...
 * 世界天气/事件、玩家声望等级，生成一句贴合语境的台词。作为对既定脚本对话
 * 的增强补充，让 NPC 世界更具生命力。AI 失败返回保底台词。
 *
...
async function aiNpcBanter({ npcName, npcRole, taskHint, playerTitle, playerLevel, weather, activeEvents }) {
  const ctx = {
    NPC: npcName,
    身份: npcRole || '普通百姓',
  ...
  const prompt = `你是文字网游《纵横四海》里的一位 NPC（${npcName}）。根据以下情境，说一句 40 字以内的中文台词，
可以提及当前任务、天气、或这个世界正在发生的事，符合你的身份口吻。只输出台词本身，不要前缀：
${JSON.stringify(ctx)}`;
  try {

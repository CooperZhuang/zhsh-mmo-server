...
$script:web = Split-Path -Parent $MyInvocation.MyCommand.Path
$script:serverJs = Join-Path $script:web 'game-server.js'
$script:lock = Join-Path $script:web '.zhsh-game-server.lock'
$script:port = 20109
...
function Ensure-FirewallRule {
  ...
  if ($LASTEXITCODE -ne 0) {
    try {
      Start-Process -FilePath 'netsh' -ArgumentList 'advfirewall','firewall','add','rule','name=Zhsh Game Server','dir=in','action=allow','protocol=TCP','localport=20109' -Verb RunAs -Wait -WindowStyle Hidden
    } catch { }
  }
}
...
function Start-Server {
  ...
  Start-Process -FilePath $node.Source -ArgumentList "`"$script:serverJs`"" -WorkingDirectory $script:web -WindowStyle Hidden
